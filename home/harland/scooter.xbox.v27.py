...
# uses xbox joystick for control of wheels and head
...
def ZButtonListener(value):
  ...
  if (value > 0.95):
    serial.write("HDLT 20\r")		#set lights
    if speed >19:
      speed = 10			#set to slow for running into base
      print "speed=12"
      i01.mouth.speak("speed slow")
      sleep(2)
    else:
     speed = 25
     print "speed= 25"
     i01.mouth.speak("speed normal")
     sleep(10)
  if (value < -0.95):
    serial.write("HDLT 00\r")
    speed = 35
    print "speed= 35"
    i01.mouth.speak("speed fast")
    sleep(2)
...
def LButtonListener(value):
   if value == 1.0:
      print("lft-but scooter at rest")
 #     i01.mouth.speak("Scooter is taking a break")
      i01.powerDown()
      serial.write("HDLT 40\r")			#red leds on showing at rest
      sleep(60)					#let stuff become calm so does not retrigger
...
# head neck 50-170, rotatehead 10-170, eyeX, eyeY 5-60, jaw 50-110
      ...
      batvolts()
      irsensors()
#reset distance counter which software does not use yet
      serial.write("RST\r")
      getping()
