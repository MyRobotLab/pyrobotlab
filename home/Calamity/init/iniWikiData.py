...
def askWiki(query):
	...
	if wikiAnswer == "Not Found !":
		answer = "I don't know"
	pinocchio.getResponse("say " + answer)
...
def getProperty(query, what):
	...
	propertiesFile = wikiDataPath + u"/properties_ID.txt"
	f = codecs.open(propertiesFile,'r',"utf-8") # set you propertiesID.txt path !!
	for line in f:
...
def getDate(query, ID): # this function return a custom display
	...
	pinocchio.getResponse("say The " + answer)
