...
import static org.myrobotlab.codec.serial.ArduinoMsgCodec.PIN_MODE;
import static org.myrobotlab.codec.serial.ArduinoMsgCodec.PROCESS_MESSAGE;
import static org.myrobotlab.codec.serial.ArduinoMsgCodec.PUBLISH_CUSTOM_MSG;
...
import static org.myrobotlab.codec.serial.ArduinoMsgCodec.RESOLVE_CUSTOM_MESSAGE;
import static org.myrobotlab.codec.serial.ArduinoMsgCodec.RESOLVE_SENSOR_DATA;
...
import org.myrobotlab.service.data.Sensor;
...
public class Arduino extends Service implements Microcontroller, I2CControl, SerialDataListener, ServoController, MotorController {
   ...
   */
  HashMap<Integer, Sensor> indexToSensor = new HashMap<Integer, Sensor>();
  ...
  public void afSetPwmFreq(){

  }

  public void afSetPwm(){

	}

	public void afSetServo(){
  ...
  private void processMessage(int[] message) {
    ...
    switch (function) {
      ...
      case PUBLISH_SENSOR_DATA: {
    	// if its out data (Arduino pin data)
    	// then we update the pin value and publish the pin
        // get the sensor callback index
    	// PUBLISH_SENSOR_DATA - should come back in the form

    	// find the correct sensor with the sensor index
        ...
        // get the sensor
        Sensor sensor = indexToSensor.get(sensorIndex);

        // unload the data
        sensor.sensorData.data = new int[size];
        for (int i = 0; i < size; ++i){
        	sensor.sensorData.data[i] = message[2 + 1];
        }
        ...
        if (sensor.callback != null){
        	sensor.callback.update(sensor.sensorData);
        }
        // potentially, there could be a } else { here
        // which would switch between optimized .update and regular(network capable) publishSensorData
        // we will attempt to do both at the moment .. and see how the consumers like that

        // publish

        invoke("publishSensorData", sensor.sensorData);
  ...
  /* NO LONGER NEEDED OR WANTED - publishSensorData(int[]) handles it all - the service
   * which consumes SensorData does the conversion
   *
...
*/
  ...
  /* No longer needed publishSensorData(int[]) works for all
  ...
  */
  ...
  public synchronized boolean sensorAttach(SensorDataPublisher sensor) {
    ...
    Sensor sd = new Sensor(sensorIndex, sensor);
