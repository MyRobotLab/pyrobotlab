...
public class Arduino extends Service implements Microcontroller, I2CControl, SerialDataListener, ServoController, MotorController, SensorDataPublisher {
	 ...
	 */
	HashMap<Integer, Device> indexToDevice = new HashMap<Integer, Device>();
	...
	private void processMessage(int[] message) {
		...
		case PUBLISH_SENSOR_DATA: {
			...
			// get the sensor
			Sensor sensor = indexToDevice.get(sensorIndex);
		...
		case PUBLISH_PULSE_STOP: {
			...
			// SensorDataPublisher sensor = indexToDevice.get(index).sensor;
	...
	/*
	 * No longer needed publishSensorData(int[]) works for all public Object[]
	 * publishCustomMsg(Object[] data) { return data; }
	 */
	...
	public synchronized void attachDevice(Device device) {
		...
		//     Java-land Service <----name---- deviceList <-----index----- MRLComm
		//
		// The Java-land Service owns the name, the MRLComm owns the index and Arduino owns
    ...
    int sensorIndex = deviceList.size();
    ...
    if (deviceList.containsKey(sensorName)) {
